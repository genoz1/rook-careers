...
const {prepareJob}=require('./v7Location'),{resolveLocation,classifyLocation,VERSION}=require('./jobLocationScope');
...
function reviewed(kind,states=[]){const j=job('Remote, US',null,null,null);j.location_evidence={...j.location_evidence,version:VERSION,status:'validated',source_title:j.title_original,source_description_hash:require('./jobLocationScope').descriptionHash(j.description_text),scope:{kind,states}};return j;}
...
(async()=>{
  ...
  assert.equal(classifyLocation({...j,title_original:'Account Executive',location_raw:'Boston or Remote',location_evidence:{status:'unresolved'}}).kind,'unresolved');
