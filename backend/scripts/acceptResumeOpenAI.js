...
async function run(){
 ...
 const form=new FormData();form.append('resume',new Blob([await docx()],{type:'application/vnd.openxmlformats-officedocument.wordprocessingml.document'}),'synthetic-acceptance.docx');
 ...
 const stagedForm=new FormData();stagedForm.append('resume',new Blob([await staged.arrayBuffer()],{type:staged.headers.get('X-Resume-Type')}),'synthetic-acceptance.docx');
 ...
 console.log('ACCEPTANCE_PASS',JSON.stringify({model:'gpt-4o-mini',upload:true,extraction:true,persistence:true,profileToV7:true,matches:snapshot.jobs.length,candidateFitMatches:matches.filter(j=>j.match.candidate_fit!=null).length,anthropicDisabledForDirectAnalysis:true,storedFacts:stored.resume_structured}));
...
run().catch(error=>{console.error('ACCEPTANCE_FAILED',error.message);process.exitCode=1}).finally(async()=>{
 if(userId){
  ...
  check(await db.auth.admin.deleteUser(userId));console.log('CLEANUP_PASS synthetic account and files removed');
 }
