...
global.fetch=async(url,options)=>{
 ...
 return {ok:true,json:async()=>({status:'completed',output:[{content:[{type:'output_text',text:mode==='malformed'?'not json':JSON.stringify(mode==='ungrounded'?{...structured,employers:[{...structured.employers[0],achievements:'Managed 12 direct reports.'}]}:structured)}]}]})};
};
...
test('multipart upload → extraction → actual OpenAI client → persistence → V7 matches; failed uploads preserve valid facts',async()=>{
 ...
 const upload=async(type='application/pdf',buffer)=>{
 ...
 try {
  ...
  for(mode of ['error','malformed','ungrounded']) {const failed=await upload();assert.equal(failed.analysis_status,'failed');assert.equal(failed.resume_structured,null);assert.deepEqual(profile.resume_structured,structured);}
